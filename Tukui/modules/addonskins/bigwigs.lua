...
if not C["addonskins"].bigwigs == true or not IsAddOnLoaded("BigWigs") then return end
	...
	if T.CheckAddOnShown() == true then
		if C["chat"].background == true and T.ChatRightShown == true then
			if T.RightChat == true then
				BigWigsAnchor:Point("BOTTOM", ChatRBackground, "TOP", 13, 3)	
			else
				BigWigsAnchor:Point("BOTTOM", ChatRBackground, "TOP", 13, -22)
			end
		else
			BigWigsAnchor:Point("BOTTOM", ChatRBackground, "TOP", 13, -22)	
		end	
...
f:SetScript("OnEvent", function(self, event, addon)
	...
	elseif event == "PLAYER_ENTERING_WORLD" then
		...
		if Skada and Skada:GetWindows() and Skada:GetWindows()[1] and C["addonskins"].embed == "Skada" then
		...
		elseif Recount_MainWindow and C["addonskins"].embed == "Recount" then
		...
		elseif OmenAnchor and C["addonskins"].embed == "Omen" then
